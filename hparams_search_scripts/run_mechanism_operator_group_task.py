...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Run a grouped operator task for a YAML-driven search job.')
    ...
    parser.add_argument('--candidate_id', type=int, dest='candidate_ids', action='append', required=True,
                        help='candidate id to execute inside this grouped operator task (repeat for multiple candidates)')
...
def main() -> None:
    ...
    if repeats != 1:
        raise ValueError(f'operator grouped stage runner expects repeats=1, got repeats={repeats}')
    ...
    if str(getattr(reference_args, 'feature', '')).strip().lower() != 'operator':
        raise ValueError('operator grouped runner is only intended for feature=operator paths')
    ...
    for candidate in candidates:
        ...
        if not mechanism_stage_utils.candidate_result_exists(output_dir, candidate, expected_seed=seed):
            raise RuntimeError(
                f'Grouped {cli_args.stage} operator task finished but produced no matching CSV under {output_dir} '
