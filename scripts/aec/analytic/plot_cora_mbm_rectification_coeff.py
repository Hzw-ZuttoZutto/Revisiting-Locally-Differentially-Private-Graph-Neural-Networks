...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot MBM rectification coefficient for Cora with m=best sampling."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def rectification_coeff(epsilon: np.ndarray | float, m: np.ndarray | int) -> np.ndarray:
    eps = np.asarray(epsilon, dtype=float)
    m_arr = np.asarray(m, dtype=float)
    eps_per_dim = eps / m_arr
    expm1_value = np.expm1(eps_per_dim)
    ratio = (expm1_value + 2.0) / expm1_value
    return float(CORA_FEATURE_DIM) * ratio / m_arr
...
def validate_values(epsilon: np.ndarray, m: np.ndarray, coeff: np.ndarray) -> None:
    ...
    if not np.all(np.isfinite(coeff)) or not np.all(coeff > 0):
        raise RuntimeError("All rectification coefficient values must be finite and positive")


def write_curve_csv(path: Path, epsilon: np.ndarray, m: np.ndarray, coeff: np.ndarray) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=["epsilon", "m", "rectification_coeff"])
        writer.writeheader()
        for eps_value, m_value, coeff_value in zip(epsilon, m, coeff):
            writer.writerow(
                {
                    "epsilon": f"{float(eps_value):.17g}",
                    "m": int(m_value),
                    "rectification_coeff": f"{float(coeff_value):.17g}",
                }
            )
...
def write_marked_csv(path: Path) -> list[dict[str, Any]]:
    ...
    for eps_value in MARKED_EPS:
        ...
        coeff_value = float(rectification_coeff(np.asarray([eps_value], dtype=float), m_value)[0])
        rows.append(
            {
                "epsilon": f"{eps_value:.17g}",
                "m": m_value,
                "rectification_coeff": f"{coeff_value:.17g}",
                "rectification_coeff_label": format_sci(coeff_value),
            }
        )
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=["epsilon", "m", "rectification_coeff", "rectification_coeff_label"])
        writer.writeheader()
        writer.writerows(rows)
    return rows


def plot_curve(output_dir: Path, epsilon: np.ndarray, coeff: np.ndarray, marked_rows: list[dict[str, Any]]) -> None:
    fig, ax = plt.subplots(figsize=(6.25, 4.45))
    fig.subplots_adjust(left=0.16, right=0.975, top=0.965, bottom=0.19)

    ax.plot(epsilon, coeff, color=CURVE_COLOR, linewidth=2.6, solid_capstyle="round")

    marked_x = np.asarray([float(row["epsilon"]) for row in marked_rows], dtype=float)
    marked_y = np.asarray([float(row["rectification_coeff"]) for row in marked_rows], dtype=float)
    ax.scatter(
        marked_x,
        marked_y,
        s=88,
    ...
    for row in marked_rows:
        ...
        coeff_value = float(row["rectification_coeff"])
        label = rf"$({format_eps(eps_value)}, {format_sci(coeff_value)})$"
        ax.annotate(
            label,
            xy=(eps_value, coeff_value),
    ...
    ax.set_ylim(bottom=float(np.min(coeff)) * 0.75, top=float(np.max(coeff)) * 1.8)
    ax.set_xlabel(r"Privacy budget $\epsilon$", fontsize=18, labelpad=8)
    ax.set_ylabel(r"$\beta_{\epsilon,d}$", fontsize=18, labelpad=9)
    ...
    pdf_path = output_dir / "cora_mbm_rectification_coeff.pdf"
    png_path = output_dir / "cora_mbm_rectification_coeff.png"
...
def main() -> None:
    ...
    coeff = rectification_coeff(epsilon, m)
    validate_values(epsilon, m, coeff)

    curve_csv = args.output_dir / "cora_mbm_rectification_coeff_curve.csv"
    marked_csv = args.output_dir / "cora_mbm_rectification_coeff_marked_points.csv"
    write_curve_csv(curve_csv, epsilon, m, coeff)
    marked_rows = write_marked_csv(marked_csv)
    plot_curve(args.output_dir, epsilon, coeff, marked_rows)
