...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot HDS alpha and lambda against epsilon for Cora.")
...
def hds_terms(
    ...
    eps = np.asarray(epsilon, dtype=np.longdouble)
    m_arr = np.asarray(m, dtype=np.longdouble)
    s = eps / m_arr
    exp_s = np.exp(s)
    expm1_s = np.expm1(s)

    # Algebraically identical to the displayed definitions, evaluated in
    # long-double precision to avoid cancellation near epsilon=1e-3.
    b_s = (s + (s - 1.0) * expm1_s) / (exp_s * (expm1_s - s))
    rho_s = 1.0 + np.expm1(-s) / s
    v_s = (
        b_s**3 * exp_s + 3.0 * b_s**2 + 3.0 * b_s + 1.0
    ) / (3.0 * (b_s * exp_s + 1.0))

    alpha = (np.longdouble(CORA_FEATURE_DIM) ** 2) * v_s / (m_arr * rho_s**2)
    lambda_value = np.longdouble(CORA_FEATURE_DIM) / (m_arr * rho_s)
    return (
        np.asarray(alpha, dtype=float),
        np.asarray(lambda_value, dtype=float),
        np.asarray(s, dtype=float),
        np.asarray(b_s, dtype=float),
        np.asarray(rho_s, dtype=float),
    )
...
def validate_values(
    ...
    for name, values in (
        ...
        if not np.all(np.isfinite(values)) or not np.all(values > 0):
            raise RuntimeError(f"All {name} values must be finite and positive")
...
def write_curve_csv(
    ...
    fields = ["epsilon", "m", "s", "b_s", "rho_s", "alpha_hds", "lambda_hds"]
    with path.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=fields)
        writer.writeheader()
        for values in zip(epsilon, m, s, b_s, rho_s, alpha, lambda_value):
            eps, m_value, s_value, b_value, rho_value, alpha_value, lambda_item = values
            writer.writerow(
                {
                    "epsilon": f"{float(eps):.17g}",
                    "m": int(m_value),
                    "s": f"{float(s_value):.17g}",
                    "b_s": f"{float(b_value):.17g}",
                    "rho_s": f"{float(rho_value):.17g}",
                    "alpha_hds": f"{float(alpha_value):.17g}",
                    "lambda_hds": f"{float(lambda_item):.17g}",
                }
            )
...
def plot_curve(
    ...
    ax.plot(
        ...
        label=r"$\alpha_{\epsilon,m}^{\mathrm{HDS}}$",
    )
    ax.plot(
        epsilon,
        lambda_value,
        ...
        linewidth=2.6,
        solid_capstyle="round",
        label=r"$\lambda_{\epsilon,m}^{\mathrm{HDS}}$",
    )

    marked_x = np.asarray(MARKED_EPS, dtype=float)
    marked_m = resolve_m(marked_x)
    marked_alpha, marked_lambda, _, _, _ = hds_terms(marked_x, marked_m)
    for values, color in ((marked_alpha, ALPHA_COLOR), (marked_lambda, LAMBDA_COLOR)):
        ax.scatter(
            marked_x,
            values,
            s=88,
    ...
    note = r"$d=1433$" + "\n" + r"$m=\max(1,\min(d,\lfloor\epsilon/3.317\rfloor))$"
    ax.text(
        0.035,
        0.065,
        note,
    ...
    pdf_path = output_dir / "cora_hds_alpha_lambda_epsilon.pdf"
    png_path = output_dir / "cora_hds_alpha_lambda_epsilon.png"
...
def main() -> None:
    ...
    m = resolve_m(epsilon)
    alpha, lambda_value, s, b_s, rho_s = hds_terms(epsilon, m)
    validate_values(epsilon, m, alpha, lambda_value, b_s, rho_s)
    curve_csv = args.output_dir / "cora_hds_alpha_lambda_epsilon_curve.csv"
    write_curve_csv(curve_csv, epsilon, m, alpha, lambda_value, s, b_s, rho_s)
